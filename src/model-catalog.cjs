...
function modelCatalog(store,getMembers){let data=store.read('model-catalog',{});return {
 list(){return getMembers().filter(m=>m.kind!=='terminal'&&!(m.kind==='web'&&m.webMode==='automation')).map(m=>({memberId:m.id,baseUrl:m.baseUrl||'',format:m.format||'',name:m.name,company:company(m),kind:m.kind,models:m.kind==='web'?[]:[...new Set([m.model,...(data[m.id]?.signature===signature(m)?data[m.id].models:[])].filter(Boolean))]}));},
