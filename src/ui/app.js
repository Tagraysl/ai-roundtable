...
function renderMembers(){
  ...
  for(const m of state.members){const unavailable=m.kind==='terminal'||(m.kind==='web'&&m.webMode==='automation');if(unavailable)selected.delete(m.id);const card=el('label',undefined,'member-card'+(selected.has(m.id)?' selected':''));card.append(avatar(m));const info=el('div');info.append(el('strong',m.name),el('small',kinds[m.kind]+' · '+(m.kind==='web'?(m.webMode!=='automation'?'手动接力':m.connected?'已连接':'待连接'):m.kind==='api'?(m.hasKey?'已配置':'待配置'):'可检查连接')));card.append(info);const check=document.createElement('input');check.type='checkbox';check.checked=selected.has(m.id);check.disabled=!!state.active||unavailable;if(unavailable)info.append(el('small','此接入暂不开放，请修改成员设置'));check.onchange=()=>{check.checked?selected.add(m.id):selected.delete(m.id);renderMembers();};card.append(check);$('members').append(card);}
  count();
}
function count(){$('call-count').textContent=`最多 ${selected.size*Number($('rounds').value)} 次回复`;}
function renderRooms(){$('rooms').replaceChildren();for(const r of state.rooms){const row=el('div',undefined,'room-row'),b=el('button',r.title,'room'+(r.id===roomId?' active':''));b.append(el('small',`${r.messages.filter(m=>m.role==='assistant'&&m.status==='complete').length} 条回复`));b.onclick=()=>{roomId=r.id;$('discussion-nav')?.click();render();};const remove=el('button','×','room-delete');remove.title='删除讨论：'+r.title;remove.setAttribute('aria-label',remove.title);remove.onclick=async()=>{try{state=await call('deleteRoom',{id:r.id});render();}catch(e){notify(e.message);}};row.append(b,remove);$('rooms').append(row);}}
...
function renderMessages(){const area=$('messages'),nearBottom=area.scrollHeight-area.scrollTop-area.clientHeight<100;if(!room().messages.length){area.innerHTML=welcome;bindExamples();return;}area.replaceChildren();for(const m of room().messages){const line=el('article',undefined,'message '+m.role);line.dataset.messageId=m.id;if(m.role!=='user')line.append(avatar(state.members.find(x=>x.id===m.memberId)||{kind:'api'}));const content=el('div',undefined,'message-content');const meta=el('div',undefined,'message-meta');meta.append(el('strong',(m.name||'你')+(m.model?' / '+m.model:'')),el('span',m.role==='assistant'?(m.mode==='workflow'?'工作流节点':m.mode==='summary'?'讨论汇总':`第 ${m.round} 轮`):'发起讨论'));if(m.status==='running')meta.append(el('span',m.progress||'等待成员响应…','status-tag'));else if(m.status==='cancelled')meta.append(el('span','已停止','status-tag'));if(m.omitted)meta.append(el('span',`已省略 ${m.omitted} 条较早上下文`,'status-tag'));const copy=el('button','复制','copy');copy.onclick=()=>navigator.clipboard.writeText(m.text).catch(e=>notify(e.message));meta.append(copy);content.append(meta);if(m.skills?.length)content.append(el('small','Skills: '+m.skills.map(s=>s.name+' v'+s.revision).join(' · '),'skill-usage'));const body=el('div',undefined,'message-body');renderText(body,m.text|| (m.status==='running'?(m.progress||'等待成员响应…'):''));content.append(body);if(m.error)content.append(el('div',m.error,'message-error'));if(m.mode==='workflow'&&['error','cancelled'].includes(m.status)){const retry=el('button','重试此积木','message-retry');retry.disabled=!!state.active;retry.title='只重试此积木，并继续依赖它的步骤。其他失败积木保留，需分别重试。';retry.onclick=async()=>{if(!confirm('重试此积木并继续其依赖步骤？其他失败积木不会重新请求，已完成结果保留。本积木若仅有部分回答，将重新生成，可能产生调用费用。'))return;retry.disabled=true;try{state=await call('roomWorkflowRetry',{id:roomId,runId:m.runId,nodeId:m.nodeId});render();}catch(e){notify(e.message);retry.disabled=false;}};content.append(retry);}line.append(content);area.append(line);}if(nearBottom)area.scrollTop=area.scrollHeight;}
function render(){if(!state.rooms.some(r=>r.id===roomId))roomId=state.rooms[0].id;renderRooms();renderMembers();$('room-title').textContent=room().title;renderMessages();$('materials').replaceChildren();for(const m of room().materials||[]){const chip=el('span',m.name,'material'),b=el('button','×');b.disabled=!!state.active;b.onclick=async()=>{try{state=await call('removeMaterial',{id:roomId,materialId:m.id});render();}catch(e){notify(e.message);}};chip.append(b);$('materials').append(chip);}renderBusy();window.runErrorUI?.refresh();}
...
async function refresh(){state=await call('state');if(!roomId){roomId=state.rooms[0].id;selected=new Set(state.members.map(m=>m.id));}render();}
async function send(mode='discussion',forcedIds){
  ...
  if(!forcedIds&&mentions.length){if(mentions.includes('all'))ids=state.members.map(m=>m.id);else{const matched=state.members.filter(m=>mentions.includes(m.id.toLowerCase())||mentions.includes(m.name.toLowerCase()));if(matched.length)ids=matched.map(m=>m.id);}}
...
function renderMemberList(){$('member-list').replaceChildren();for(const m of state.members){const b=el('button',m.name,m.id===editingId?'active':'');b.onclick=()=>editMember(m.id);$('member-list').append(b);}}
...
bind('summary',()=>{$('summary-member').replaceChildren();for(const m of state.members){const o=el('option',m.name);o.value=m.id;o.selected=m.id==='codex';$('summary-member').append(o);}$('summary-dialog').showModal();});
...
setInterval(async()=>{try{const s=await call('state');if(state&&JSON.stringify(state.members)!==JSON.stringify(s.members)){state.members=s.members;renderMembers();}}catch{}},3000);
