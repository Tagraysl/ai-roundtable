...
class ApiAdapter {
  ...
  async sendOnce({member,prompt,signal,onText=()=>{},onStatus=()=>{},images=[]}) {
    ...
    await readSSE(res, raw=>{
      ...
      if(data.type==='message_stop' || data.choices?.[0]?.finish_reason) finished=true;
  ...
  async sendWithTools(member,body,headers,signal,onText,onStatus){
    ...
    for(let round=0;round<4;round++){
      ...
      const data=await recover(async()=>{const res=await this.fetchImpl(endpoint(member.baseUrl,'chat/completions'),{method:'POST',headers,body:JSON.stringify(body),signal,redirect:'error'});if(!res.ok){await res.body?.cancel();throw httpError(res);}return res.json();},{signal,onStatus});
      const choice=data.choices?.[0],message=choice?.message;
      if(!message||data.error)throw Error('联网模式 API 响应格式不兼容。');
      if(choice.finish_reason==='length')throw Error('回复达到输出长度限制。');
      const calls=message.tool_calls||[];
      if(!calls.length){if(typeof message.content!=='string')throw Error('API 未返回文字回答。');const text=message.content+(sources.size?'\n\n联网来源：\n'+[...sources].join('\n'):'');onText(text);return {text,usage:data.usage};}
      if(round===3||calls.length>4)throw Error('已达到本次联网调用上限，请缩小问题范围。');
      body.messages.push(message);
      for(const call of calls){onStatus('正在查询联网资料：'+call.function?.name+'…');let result;try{if(!definitions.some(t=>t.function.name===call.function?.name))throw Error('未知工具');result=await this.network.run(call.function.name,JSON.parse(call.function.arguments),signal);if(result.url)sources.add(result.url);for(const r of result.results||[])if(typeof r.url==='string'&&/^https?:\/\//.test(r.url))sources.add(r.url);}catch(e){signal.throwIfAborted();result={error:e.message};}body.messages.push({role:'tool',tool_call_id:call.id,content:JSON.stringify(result)});}
    }
    throw Error('联网调用未完成。');
  }
}
