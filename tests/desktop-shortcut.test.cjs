...
test('shortcut quotes through structured API, preserves unrelated link and uses executable icon',()=>{
 ...
 const result=create({shell,desktop,executable});assert.equal(path.basename(result),'AI Roundtable (2).lnk');assert.equal(captured.options.target,executable);assert.equal(captured.options.icon,executable);assert.equal(captured.options.cwd,path.dirname(executable));assert.equal(fs.readFileSync(path.join(desktop,'AI Roundtable.lnk'),'utf8'),'other');
