...
foreach ($entry in @('release.ps1','verify-release.cjs','Create-Desktop-Shortcut.ps1','Create-Desktop-Shortcut.cmd','build-installer.ps1','installer.iss','build-macos.py','mac-smoke.cjs','upload-release-draft.py')) { Copy-Item -LiteralPath (Join-Path $PSScriptRoot $entry) -Destination (Join-Path $source 'scripts') }
